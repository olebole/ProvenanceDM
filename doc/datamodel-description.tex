...
\begin{figure}[hbt]
\centering
%\includegraphics[width=1.0\textwidth]{IVOAModel_VODML_Overview_2019-03.png}
%Mireille: updated with corrected cardinality for parameter
\includegraphics[width=1.0\textwidth]{figures/PR2-Overview-20190507.png}
\caption[Overview class diagram of the IVOA Provenance Data Model]{Overview class diagram of the IVOA Provenance Data Model. The core part in yellow is based on W3C PROV definitions where relations are shown in grey. It is extended by a Description part (orange), specific types of entities (dark yellow) and an Activity Configuration package (green). A full diagram with attributes is shown in Appendix~\ref{sec:fulldiagram}, Figure~\ref{fig:fulldiagram}}
\label{fig:overview}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=1.0\textwidth]{CoreModel_VODML_2019-03.png}
\caption[Core classes and relations of the IVOA Provenance DM]{Core classes and relations of the IVOA Provenance DM. Attributes for these classes are detailed in table found in the following sections.}
\label{fig:coreclasses}
\end{figure}



\subsubsection{Entity and Collection}
\label{sec:Entity}
...
location    & string & a path or a geographical location, e.g. a URL\\
%value       & prov:value &  & provides a value that is a direct representation of the entity \\
generated AtTime  & datetime & date and time at which the entity was created (e.g. timestamp of a file)\\
invalidated AtTime  & datetime & date and time of the destruction, cessation, or expiry of the entity. The entity is no longer available for use (or further invalidation) after invalidation\\
...
Each entity is usually a result from an activity, expressed by a link from the entity to its generating activity, and can be used as input for (many) other activities.
...
Tracking those relations thus answers one of the main objective of the model (see goal A in Section~\ref{sec:goals}).
...
The \class{Used} relation is closely coupled to the \class{Activity} by a composition, indicated in Figure~\ref{fig:coreclasses} by a filled diamond. If an activity is deleted, then the corresponding \class{Used} relations need to be removed as well. The entities that were used still remain, since they may have been used for other activities as well. The multiplicity is * between \class{Used} and \class{Entity}, because an entity can be used more than once (by different activities).
...
The \class{WasGeneratedBy} relation is closely coupled with the \class{Entity} via a composition, as indicated in Figure~\ref{fig:coreclasses} by a filled diamond. If an entity is deleted, then its \class{WasGeneratedBy} relation must be deleted as well. There is a multiplicity * between \class{Activity} and \class{WasGeneratedBy}, because an activity can generate many entities. However, an entity can be generated by only one activity, so the multiplicity is 1 or 0 between \class{Entity} and \class{WasGeneratedBy}.
...
Roles in Entity-Activity relations are free text attributes, but may contain reserved words in the future to foster interoperability.
%, except for predefined words listed in a dedicated vocabulary webpage.

The \attribute{role} cannot be an attribute of the \class{Entity} class, since the same entity (e.g. a specific file containing an image) may play different roles with different activities. If this is not the case, if the image can only play the same role everywhere, only then it can be an intrinsic property of the entity.
...
This relation makes it possible to visualize independently the flow of activities as it occurred, which may be the result of the execution of a workflow. It does not need a priori a specific class or table in an implementation, but it provides a way to expose partial information that follow the general chain \class{Used-Entity-WasGeneratedBy} where the entity may be an empty instance because it is unknown or irrelevant.
...
The \class{Agent} class in the model have the attributes given in Table \ref{tab:agent}. 

An Agent is generally someone who pressed a button, ran a script, performed the observation or published a dataset. The agent can be a single person, a group of persons, a project or an institute. For example: telescope astronomer, observatory, pipeline operator, principal investigator, software engineer, project helpdesk.
...
type        & string & type of the agent as given in Table~\ref{tab:agent-types}\\
...
\small
\tymax  0.5\textwidth
\textbf{\normalsize Agent types}\vspace{0.25em}\\
\begin{tabulary}{1.0\textwidth}{lp{9cm}}
\toprule
\head{Class or type} &\head{Comment} \\
\midrule
...
\bottomrule
\end{tabulary}
\caption[Agent class and types of sub-classes]{Agent class and types of sub-classes.}
...
%It is desirable to have at least one agent given for each activity.
There can also be more than one agent for each activity with a different role and one agent can be responsible for more than one activity or entity, using the relations defined in the following sections.
...
Roles in relations to Agent are free text attributes, except for predefined words listed in Table \ref{tab:agent-roles}.
...
\begin{tabulary}{1.0\textwidth}{llL}
\toprule
\head{role} & \head{Label} & \head{Comment} \\
\midrule
author      & Author & The agent is an author of this entity (e.g. article, software, proposal)\\
operator    & Operator & The agent was an operator for a specific activity \\ % removed executor: ambiguous
coordinator & Coordinator & The agent was coordinating a specific activity \\ % we should choose one word : PI?
observer    & Observer & The agent was observing and is associated to a specific “observation” activity or responsible for observing an specific entity \\
contributor & Contributor & sThe agent helped in the creation of this entity \\
creator     & Creator & The agent created this entity (creators of articles or software are rather called ``author'') \\
curator     & Curator & The agent curated this entity \\
funder      & Funder & The agent provided financial support for this activity or the creation of the entity \\
provider    & Provider & The agent provided this entity \\
publisher   & Publisher & The agent publishes this entity \\
editor      & Editor & The agent was responsible for editing this entity \\
%(owner) & Owner & Does anyone really own the data?\\
\bottomrule
\end{tabulary}
\caption[Predefined words for roles of agents]{Predefined words for roles of agents}
...
\begin{figure}[ht]
\centering
\includegraphics[width=1.0\textwidth]{IVOAModel_VODML_Descriptions_2019-03.png}
...
% 2018-12 commented
%A similar normalization of descriptions of the actual processes and datasets can be found in the IVOA Simulation DM \citep[SimDM, ][]{2012ivoa.spec.0503L}}), which describes simulation metadata. The SimDM classes \class{Experiment} and \class{Protocol} correspond to the Provenance terms \class{Activity} and \class{ActivityDescription}.
...
The activity \attribute{type} is a free text attribute, except for predefined words listed in Table \ref{tab:activitydescription-roles}.
...
\small
\tymax  0.5\textwidth
\textbf{\normalsize ActivityDescription.type}\vspace{0.25em}\\
\begin{tabulary}{1.0\textwidth}{llL}
\toprule
\head{role} & \head{Label} & \head{Comment} \\
\midrule
...
\bottomrule
\end{tabulary}
\caption[Predefined words for types of activities]{Predefined words for  types of activities}
...
If a \class{UsageDescription} (respectively \class{GenerationDescription}) instance is defined, the \attribute{role} attribute of the related \class{Used} (respectively \class{WasGeneratedBy}) instances must point to the \attribute{role} attribute of this \class{UsageDescription} (respectively \class{GenerationDescription}) instance.
...
%For example
It is a free text attribute, except for predefined words listed in Table~\ref{tab:usage-generation-types}. 
...
%option  & information passed to an activity to specify its execution, examples: algorithm, stack_method, overwrite, ...\\
...
\bottomrule
\end{tabulary}
\caption[Types of Usage and Generation]{Types of Usage and Generation.}
...
We anticipate that other types of entities can be defined for specific projects (for example, a device, a document, a vizualization...). The \attribute{type} attribute of the \class{EntityDescription} class should be used to differentiate the different categories of entities.
...
default       & (value dependent) & the default value of the value entity, type depends on \attribute{valueType} \\
\bottomrule
\end{tabulary}
...
% 2019-03 commented
% The concept of parameter is very general, and already exists in the VO context in different forms, for example as PARAM elements in VOTable \citep{std:VOTABLE}, as POST/GET parameters for web services \citep[see for example][for SODA services]{2017ivoa.spec.0517B} or as uws:param elements in the UWS pattern \citep{std:UWS}. 
% The Simulation DM also contains a \class{ParameterSetting} class \citep{2012ivoa.spec.0503L}}.
